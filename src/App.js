...
function App() {
  ...
  const fetchDataBasedOnFilter = async () => {
    ...
    try {
      ...
      const responses = await Promise.all(apiEndpoints.map(endpoint => axios.post(`${API_URL}/${endpoint}`)));
                        ...
                        <div onClick={fetchDataBasedOnFilter} className="flex-auto p-4 hover:cursor-pointer">
                          <div className="flex flex-wrap">
                            <div className="relative flex-1 flex-grow w-full max-w-full">
                              <h5 className="text-xs font-bold uppercase text-blueGray-400">Revenue Generated</h5>
                              <span className="block text-2xl font-bold">
                                {isLoading ? (
                                  // Loading animation or placeholder
                                  <div className="text-center loading-animation">Loading...</div>
                                ) : (RevenueGeneratedTotal)}
                              </span>

                            </div>
                            <div className="relative flex-initial w-auto pl-4">
                              <div className="inline-flex items-center justify-center w-12 h-12 p-3 text-center text-white bg-red-500 rounded-full shadow-lg"><svg aria-hidden="true" fill="none" viewBox="0 0 24 24" stroke="currentColor" className="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                              </svg></div>
                            </div>
                          </div>
                          <p className="mt-4 text-sm text-blueGray-500"><span className="mr-2 text-emerald-500"><i className="fas fa-arrow-up"></i> 3.48%</span><span className="whitespace-nowrap">Since last month</span></p>
                        </div>
                      </div>
                    </div>
